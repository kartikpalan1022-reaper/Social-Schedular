            ...
            {statCards.map((card)=>(
                <div key={card.label} className="bg-white hover:bg-red-50 relative border border-slate-200 rounded-2xl p-5 hover:border-red-200 transition-all">
                    <div className="flex items-center justify-between mb-4">
                        <div className="text-3xl font-medium tex-slate-800 tabular-nums">{card.value}</div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            ...
            {activities.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 px-6">
                    ...
                    <p className="text-slate-400 text-sm mt-1">Connect accounts and scheduled posts to see events here</p>
                </div>
            ) : (
